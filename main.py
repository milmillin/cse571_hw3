...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--task", type=str, default="policy_gradient", help="choose task"
    )
    ...
    if args.task == "behavior_cloning":
        ...
        policy = BCPolicy(
            obs_size, ac_size, hidden_dim=hidden_dim, hidden_depth=hidden_depth
        )  # 10 dimensional latent
        policy.to(device)
    ...
    if args.task == "dagger":
        ...
        expert_policy = torch.load(
            "data/expert_policy.pkl", map_location=torch.device(device)
        )
        ...
        if not args.test:
            ...
            simulate_policy_dagger(
                ...
                episode_length=episode_length,
                batch_size=batch_size,
                num_dagger_iters=num_dagger_iters,
